...
seu <- ScTypeAnnotation(seu, assay, reduction, f_db, tissue, outdir)
...
if (plot){
    ...
    for (group in groupset){
        p <- CustomizedPlotUMAP(obj=seu, title=title, reduction=reduction, group_by=group, label=TRUE)
        ggsave(paste0(outdir, '/umap.', group, '.', reduction, '.pdf'), width = 6, height = 4.5)
    }
}
